...
const initGame = () => {
  ...
  spinCanvas = d3
    .select(document.getElementById("spinSvg"))
    .attr("width", boxWidth)
  ...
  rotateOuter = (direction) => {
    return (d) => {
      var interpolateEnd = d3.interpolate(
        d.endAngle,
        direction == "left" ? d.endAngle - tau : d.endAngle + tau
      );
      var interpolateStart = d3.interpolate(
        d.startAngle,
        direction == "left" ? d.startAngle - tau : d.startAngle + tau
      );
...
var start = () => {
  ...
  outerTimer.restart(() => {
    outerArc
      .transition("rotateOuter")
      .duration(outerSpeed)
      .ease(d3.easeLinear)
      .attrTween("d", rotateOuter(outerDirection));

    orbit
      .transition("rotateOrbit")
      .duration(outerSpeed)
      .ease(d3.easeLinear)
...
var _handleClick = (e) => {
  ...
  setTimeout(() => {
    ...
    var outer = d3
      .select(document.querySelector("#innerArc"))
      .attr("id", "outerArc");
    var inner = d3
      .select(document.querySelector("#centerArc"))
      .attr("id", "innerArc");
    ...
    let ballAngle = Math.atan2(
      -parseFloat(spinCircle.attr("cy")),
      parseFloat(spinCircle.attr("cx"))
    );
    if (ballAngle < 0) {
    ...
    if (e > s) {
      ...
      if (
        (ballAngle - safeZone > s && ballAngle + safeZone > e) ||
        (ballAngle - safeZone < s && ballAngle + safeZone < e)
      ) {
        circleInInnerArc = true;
        points++;
